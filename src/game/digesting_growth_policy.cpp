...
segment_ptr digesting_growth_policy::growth_segment(ymse::vec2f pos, ymse::vec2f dir, float length) {
...
void digesting_growth_policy::grow(segment_sequence& body, float length) {
	ymse::vec2f pos = body.get_head_pos();
	ymse::vec2f v_dir = body.get_head_direction();
